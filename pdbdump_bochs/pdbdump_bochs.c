/* ------------------------------------------------------------------------------
 pdbdump_bochs.c - dump symbols from .pdb and executable files (public domain).
           - Martin Ridgers, pdbdump 'at' fireproofgravy.co.uk

Mod by David Reguera Garcia aka Dreg
dreg@fr33project.org - @therealdreg
https://www.fr33project.org - https://github.com/therealdreg/

CHANGELOG 26/07/2022:
    - added x64 compatibility (x64 base address)
    - added bochs syms output
...
//------------------------------------------------------------------------------
const char* g_usage =
"pdbdump_bochs.exe - dump symbols from .pdb and executable files."                 "\n"
"  Martin Ridgers, pdbdump 'at' fireproofgravy.co.uk"                        "\n"
"\n"
" Mod by David Reguera Garcia aka Dreg"                                      "\n"
" dreg@fr33project.org - @therealdreg"                                       "\n"
...
void print_symbol(const sym_info_t* sym_info)
{
    ...
    if (bochs)
    {
        fix_str(sym_info->name);
        if (NULL == prefix)
        {
            printf("0x%llx %s\n", sym_info->addr, sym_info->name);
        }
        else
        {
            printf("0x%llx %s%s\n", sym_info->addr, prefix, sym_info->name);
        }
    }
    else
    {
...
int main(int argc, char** argv)
{
    ...
    // Yes, I know, ugly and dirty as hell xD
    for (int i = 0; i < argc; i++)
    { 
        prefix = strstr(argv[i], "PFX:");
        if (NULL != prefix)
        {
            prefix += 4;
            if ('\0' == *prefix)
